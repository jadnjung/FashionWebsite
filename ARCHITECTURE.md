...
`archive/` (ROADMAP.md Phase 11) is now built — `lib/archive/`/`components/archive/` is a new top-level domain, following the same one-level-deeper convention `components/interactive-model/`/`lib/interactive-model/` established above rather than nesting under an existing domain: `lib/archive/collections.ts` isolates this feature's own Shopify dependency (`getArchivedCollections`/`getArchivedCollection`, mirroring `lib/home/selected-pieces.ts`'s shape — a small domain module wrapping the raw `lib/shopify/*` fetch with this domain's own business logic), and `components/archive/` holds the presentational components (`ArchiveIndex`, `ArchivedCollectionView`, `ArchivedProductTile`).
