...
- [x] Stage 2: product info panel works (name, category, price, colors, sizes, View/Quick Add) — real, live product data (`getProduct`), real variant selection (shared `VariantPicker`, D-036) with live availability gating, `VIEW PRODUCT` (real navigation) and `QUICK ADD` (real, disabled-until-complete, no-op `onClick` — no cart exists yet, D-016/D-029). An independent review pass found and this phase fixed a real keyboard-reachability defect and a Shop the Look sold-out gap before completion — see DECISIONS.md D-037
