...
- [ ] SEO pass (metadata, structured data, sitemap, Open Graph, canonical URLs) — includes resolving DECISIONS.md D-026: unknown-subcategory routes (`/tops/[subcategory]` etc.) currently return HTTP 200 + `noindex` instead of a hard 404 (Next.js's documented behavior for `notFound()` on a dynamically-rendered route in this version; three fix attempts were tried and empirically disproven — see D-026 for the full investigation)
