...
- [ ] Performance pass (Core Web Vitals, image loading, bundle size) — includes evaluating whether `/access`'s entrance motion should be dynamically imported: `motion`'s first use added ~119KB decoded JS to that route, and ARCHITECTURE.md §9 asks for heavy motion code to be dynamically imported. Deferred deliberately rather than applied blind, since deferring the layer would delay the SVG silhouettes' entrance until after hydration (they SSR at full opacity today) — though NOT the giant "ESQUE" typography, whose `motion.h2` `initial={{ opacity: 0, scale: 0.96 }}` is already serialized into the SSR'd inline style, so it's invisible until hydration regardless of this decision. No Core Web Vitals measurement exists yet to weigh the silhouette trade-off against. Candidate approach: Motion's `LazyMotion` + the `m` component from `motion/react-m` with the `domAnimation` feature set (per Motion's own docs, the standard `motion` component is ~34kb un-tree-shakeable versus ~4.6kb for the lazy form, with unchanged SSR output) — EntranceMotion uses no layout animation, drag, or gestures, so the swap would be mechanical. Recorded as a candidate only; not yet implemented.
