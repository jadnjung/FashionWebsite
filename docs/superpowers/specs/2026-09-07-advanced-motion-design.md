...
- **`ViewTransition` is not in `@types/react`'s default `index.d.ts`** (grep confirms zero matches) — it lives only in `experimental.d.ts`/`canary.d.ts`. Per that file's own header, it can be activated via a `"types"` array entry, an `import {} from 'react/experimental'`, or a triple-slash `/// <reference types="react/experimental" />`. A global tsconfig `"types"` array was considered and **rejected**: TypeScript's documented behavior is that setting `"types"` explicitly stops auto-including every other `@types/*` package (e.g. `@types/node`), which would silently break unrelated typechecking. `import {} from 'react/experimental'` was tried next and **also rejected** — empirically, not just by reading the docs: it typechecks (an empty import binding set doesn't obviously need a real runtime module), but it compiles to a real, literal side-effect import that Next's Turbopack bundler then tries to resolve as an actual module — `'react/experimental'` isn't one (confirmed directly: `react`'s own `package.json` `exports` map has no `./experimental` entry) — so `pnpm build` fails with "Module not found: Can't resolve 'react/experimental'". The triple-slash reference is the one mechanism that is purely a compiler directive, stripped unconditionally by every tool (tsc, SWC, Babel) and never emitted as code — this is what's actually used, per file, at the top of each file that imports `ViewTransition`.
...
**Directly verified, not just asserted**, via a temporary fixture-backed scratch route (removed before commit, mirroring D-034's own manual-verification technique): `<ViewTransition>` genuinely executes at runtime (React's own duplicate-name detection fired correctly and specifically when a test scenario artificially mounted `ProductGrid` and `ProductGallery` — which never co-render on the same real page — simultaneously with the same handle, proving both that the safety mechanism this design relies on is real and that the two components never trigger it in the actual route structure); a real `<Link>` click through a `ViewTransition`-wrapped `ProductCard` correctly updates the URL and reaches the destination route (confirmed both by network/frame-navigation instrumentation and by the existing, unmodified `catalog.spec.ts`/`pdp.spec.ts`/`homepage.spec.ts` suites continuing to pass in full afterward) — transitions do not block or delay navigation.
